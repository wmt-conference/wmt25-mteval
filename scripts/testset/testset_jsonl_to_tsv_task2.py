...
# Keep this regex for newline handling
NEWLINES_REGEX = re.compile(r"\s*\n\s*")


def trim_and_escape(text):
    """
    Cleans a string segment for safe inclusion in a TSV field.
    ...
    if not isinstance(text, str):
        return ""
    # 1. Replace tabs and carriage returns with a space first
    fixed = text.replace("\t", " ").replace("\r", " ")
    # 2. Collapse multiple whitespace characters (including spaces from above) into one
    fixed = SPACES_REGEX.sub(" ", fixed)
    # 3. Handle newlines by replacing them with a literal \n
    fixed = NEWLINES_REGEX.sub(r"\\n", fixed)
    # 4. Final trim
    return fixed.strip()
...
def get_target_systems(df, has_humaneval=True):
    """
    Gets system names based on autorank and constraints.

    Args:
        df (pd.DataFrame): DataFrame with columns ['system', 'is_constrained', 'autorank'].
        has_humaneval: has a column with human eval indication

    Returns:
        list: A sorted list of unique system names including:
              - Top 20 systems by autorank.
              - Any constrained systems within the top 30 by autorank.
    """
    if not all(col in df.columns for col in ["system", "is_constrained", "autorank"]):
        raise ValueError(
            "DataFrame must contain 'system', 'is_constrained', and 'autorank' columns."
        )
    if df["is_constrained"].dtype != "bool":
        print(
            "Warning: 'is_constrained' column is not boolean. Unexpected behavior might occur."
        )
    if has_humaneval:
        selected_systems = df[df["will_humeval"]]["system"].tolist()
        return selected_systems
    else:

        df_sorted = df.sort_values(by="autorank", ascending=True).reset_index(drop=True)

        top_20_systems = df_sorted.head(20)["system"].tolist()

        top_30_df = df_sorted.head(30)
        constrained_in_top_30 = top_30_df[top_30_df["is_constrained"] == True][
            "system"
        ].tolist()
        combined_systems = set(top_20_systems) | set(constrained_in_top_30)

        return sorted(list(combined_systems))
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Location of the directory containing documents info to be human evaluated.",
    )
    parser.add_argument(
    ...
    # read humaneval
    with open(f"{args.humaneval_info}/humaneval_info.json", "r") as f:
        humaneval_info = json.load(f)

    # Filter data based on AutoRank
    filtered_segs_by_lp = dict()
    for lp in complete_segs_by_lp:
        filtered_segs_by_lp[lp] = list()
        segments = complete_segs_by_lp[lp]
        selected_systems = humaneval_info[lp]["systems"]
        selected_docs = humaneval_info[lp]["docs"]
        for seg in segments:
            if getattr(seg, "system_id") in selected_systems and getattr(seg, "doc_id") in selected_docs:
                filtered_segs_by_lp[lp].append(seg)
    ...
    with open(args.output_path, "w", encoding="utf-8") as f:
        ...
        for lp, segments in filtered_segs_by_lp.items():
            if segments:
                for seg in segments:
                    if seg is not None:
                        count += 1
